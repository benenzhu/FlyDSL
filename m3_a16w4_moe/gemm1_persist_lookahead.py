...
"""Persistent decode GEMM with row-descriptor look-ahead under the K loop.
...
def compile_gemm1_persist(
    *, D_HIDDEN, D_INTER, NE, TOPK, BM=16, TILE_N=32, TILE_K=128,
    ...
    @flyc.kernel(name=f"gemm1_a16w4_persist_lookahead_h{K}_i{I}_e{NE}_pf{prefetch}_{w_layout}_nt{b_cache_mod}", known_block_size=[256, 1, 1])
    def kernel(
        ...
        if pid < total:
            ...
            for j, state in range(0, count, init=state0):
                ...
                for kt in range_constexpr(KT):
                    ...
                    vals = tile_load(d if future < KT else next_state["ready"], future % KT, ring[-1])
                    ring.append(vals)
