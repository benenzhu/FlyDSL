...
for pkg in moe_a16w4_decode moe_a4w4_prefill; do
...
docker exec "$CTR" bash -c "rm -rf $DST/moe_a16w4_decode/__pycache__ $DST/moe_a4w4_prefill/__pycache__"
...
case "$WHAT" in
  ...
  all)     run_suite decode test_m3_flydsl_decode_moe.py; run_suite prefill test_m3_flydsl_prefill_moe.py ;;
  ...
  *) echo "usage: $0 [decode|prefill|all|bench]"; exit 2 ;;
esac
