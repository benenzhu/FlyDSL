...
"""Decode routing sort with a wave-local prefix scan: one launch.

Block 0 sorts the (token, slot) pairs by expert: every thread loads its <= PPT pairs at
once (one round trip), LDS histogram, wave prefix sums over the padded counts,
one cross-wave exchange, then placement through LDS cursors and padding; every
other block zeroes a slice of the output buffer that gemm2's atomics accumulate into.
Same output contract as aiter ``moe_sorting`` / aiter#3832 ``sort_quant_kernel(kSkipQuant)``:

  sorted_ids[row]        = token | slot << 24     (padding rows: token = n_tokens, slot 0)
...
deterministic, as in aiter); the gemms do not depend on it. aiter's ``moe_sorting`` is two
kernels (~9.6 us together at M=64..256 in a graph).
"""
...
def _lds_atomic_add(lds_memref, elem_offset, value):
    byte_off = ArithValue(_i64(elem_offset) * arith.constant(4, type=T.i64)).index_cast(T.index)
    base = memref.extract_aligned_pointer_as_index(lds_memref)
    p = llvm.inttoptr(ir.Type.parse("!llvm.ptr<3>"), _raw(arith.index_cast(T.i64, base + byte_off)))
    return ArithValue(
        llvm.AtomicRMWOp(
            llvm.AtomicBinOp.add, p, _raw(value), llvm.AtomicOrdering.monotonic, syncscope="workgroup", alignment=4
        ).result
    )
...
def compile_decode_sort(
    *, E: int, topk: int, block_m: int, H: int, max_tokens: int = 256, zero_ctas: int = 127, threads: int = 256
):
    ...
    allocator = SmemAllocator(None, arch=get_rocm_arch(), global_sym_name=f"smem_m3_decode_sort_wave_E{E}_T{threads}")
    ...
    @flyc.kernel(name=f"m3_decode_sort_wave_zero_{tag}", known_block_size=[threads, 1, 1])
    def sort_zero(
        ...
        with ir.InsertionPoint(top.then_block):
            ...
            scan_b = SmemPtr(base_ptr, off_scan + threads * 4, T.i32, shape=(threads,)).get()
            ...
            # 3. Each wave scans 64 padded counts in registers. Only the four
            # wave totals cross LDS, with one CTA barrier instead of eight.
            cnt = ArithValue(arith.select(t_lt_E, _raw(memref.load(count, [arith.select(t_lt_E, tx, arith.index(0))])), _raw(c0_i32)))
            padded = (cnt + c_bm1) & c_bm_mask
            lane = tx_i32 & arith.constant(63, type=i32)
            wave = tx_i32 >> arith.constant(6, type=i32)
            incl = padded
            for r in range_constexpr(6):
                d = 1 << r
                has = arith.cmpi(CmpIPredicate.uge, lane, arith.constant(d, type=i32))
                j = ArithValue(arith.select(has, _raw(lane - arith.constant(d, type=i32)), _raw(lane)))
                other = ArithValue(rocdl.ds_bpermute(i32, _raw(j * arith.constant(4, type=i32)), _raw(incl)))
                incl = ArithValue(arith.select(has, _raw(incl + other), _raw(incl)))
            is_last_lane = _if(arith.cmpi(CmpIPredicate.eq, lane, arith.constant(63, type=i32)))
            with ir.InsertionPoint(is_last_lane.then_block):
                memref.store(_raw(incl), scan_b, [wave.index_cast(T.index)])
                scf.YieldOp([])
            gpu.barrier()
            for w in range_constexpr(threads // 64 - 1):
                total_w = ArithValue(memref.load(scan_b, [arith.index(w)]))
                later = arith.cmpi(CmpIPredicate.ugt, wave, arith.constant(w, type=i32))
                incl = incl + ArithValue(arith.select(later, _raw(total_w), _raw(c0_i32)))
            start = incl - padded
            end = incl
            ...
            with ir.InsertionPoint(if_e.then_block):
                ...
                if_last = _if(arith.cmpi(CmpIPredicate.eq, tx, arith.constant(E - 1, index=True)))
            ...
            for p, valid, e, w in pairs:
                ...
                with ir.InsertionPoint(if_v.then_block):
                    ...
                    packed = ((p // c_topk) & c_tok_mask) | ((p % c_topk) << c_slot_shift)
        ...
        with ir.InsertionPoint(top.else_block):
            ...
            zero_v = _arith_dialect.ConstantOp(v4_ty, ir.DenseElementsAttr.get_splat(v4_ty, ir.IntegerAttr.get(i32_ty, 0))).result
            n_chunks = arith.index_cast(idx_t, n_tok * arith.constant(H * 2 // 16, type=i32))
    ...
    def launch(
        ...
        with ir.InsertionPoint(ctx.gpu_module_body):
            allocator.finalize()
        sort_zero(topk_ids, topk_w, sorted_ids, sorted_w, sorted_eids, num_valid, out, n_tokens).launch(
            grid=(1 + zero_ctas, 1, 1), block=(threads, 1, 1), stream=stream
        )
