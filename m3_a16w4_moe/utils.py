...
try:  # flydsl <= 0.2.x (the vLLM image): raw buffer ops still live in flydsl.expr
    from flydsl.expr import buffer_ops
except ImportError:  # flydsl >= 0.3 (#880 moved them into the kernels tree)
...
def s_waitcnt_lgkm0():
    """``s_waitcnt lgkmcnt(0)`` (vmcnt/expcnt left at max). flydsl 0.2.x exposes only the
    raw simm16 form; CDNA encoding: vmcnt [3:0]+[15:14], expcnt [6:4], lgkmcnt [11:8]."""
...
def _parse_static_strides(layout):
    ...
    assert all(t != "?" for t in strides), f"crd2idx needs static strides, got {ly_str!r}"
    return [int(t) for t in strides]


def crd2idx(crd, layout):
    """Flat index of a coordinate list under a STATIC-stride layout: sum(c_i * stride_i).
    (Trimmed copy of kernels/common/layout_utils.crd2idx; our W/scale layouts are static.)"""
