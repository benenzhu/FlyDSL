...
once (one round trip), LDS histogram, a Hillis-Steele scan over the block_m-padded counts
(log2(threads) barrier rounds), then placement through LDS cursors and padding; every
...
def compile_decode_sort(
    *, E: int, topk: int, block_m: int, H: int, max_tokens: int = 256, zero_ctas: int = 127, threads: int = 256
    ...
    allocator = SmemAllocator(None, arch=get_rocm_arch(), global_sym_name=f"smem_m3_decode_sort_E{E}_T{threads}")
