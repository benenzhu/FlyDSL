...
"""Middle-batch gate/up: BM32/64, TN128, four N waves, VGPR fp4 ring.
...
def compile_moe_gemm1_mid(*, H, I, E, BLOCK_M=32, prefetch=3):
    ...
    BM, BN, KT = BLOCK_M, 128, H // 128
    MR = BM // 16
    assert 1 <= prefetch < KT

    @flyc.kernel(name=f"gemm1_a4w4_mid_h{H}_i{I}_e{E}_bm{BM}_pf{prefetch}", known_block_size=[256, 1, 1])
    def kernel(
        ...
        if mb * fx.Int32(BM) < valid_rows:
            ...
            nbase = nb * fx.Int32(BN) + wave * fx.Int32(32)
            ...
            def load_tile(kt, prev):
                aa = [bop.buffer_load(ar, (tokens[mi] * fx.Int32(H // 2) + fx.Int32(kt * 64) + q16 * fx.Int32(16)) // fx.Int32(4), vec_width=4, dtype=fx.Int32) for mi in range_constexpr(MR)]
                bb = []
                for gu in range_constexpr(2):
                    slab = []
                    for ni in range_constexpr(2):
                        nblk = expert * fx.Int32(2 * I // 16) + nbase // fx.Int32(16) + fx.Int32(gu * I // 16 + ni)
                        off = nblk * fx.Int32(H * 8) + fx.Int32(kt * 1024) + q16 * fx.Int32(256) + l16 * fx.Int32(16)
                        slab.append(bop.buffer_load(wr, off // fx.Int32(4), vec_width=4, dtype=fx.Int32))
                    bb.append(slab)
                if const_expr(kt % 2 == 0):
                    sa = [bop.buffer_load(asr, (mbase // fx.Int32(32) + fx.Int32(mp)) * fx.Int32(H // 256 * 64) + fx.Int32(kt // 2 * 64) + q16 * fx.Int32(16) + l16, vec_width=1, dtype=fx.Int32) for mp in range_constexpr(MR // 2)]
                    sb = [bop.buffer_load(wsr, (expert * fx.Int32(2 * I // 32) + nbase // fx.Int32(32) + fx.Int32(gu * I // 32)) * fx.Int32(H // 256 * 64) + fx.Int32(kt // 2 * 64) + q16 * fx.Int32(16) + l16, vec_width=1, dtype=fx.Int32) for gu in range_constexpr(2)]
                else:
                    sa, sb = prev[2], prev[3]
                return aa, bb, sa, sb

            acc = [[[fx.Vector.filled(4, 0.0, fx.Float32) for gu in range(2)] for ni in range(2)] for mi in range(MR)]
            ring = []
            ...
            for kt in range_constexpr(KT):
                ...
                for mi in range_constexpr(MR):
                    for ni in range_constexpr(2):
                        for gu in range_constexpr(2):
                            acc[mi][ni][gu] = fx.Vector(rocdl.mfma_scale_f32_16x16x128_f8f6f4(
                                T.vec(4, T.f32), [
                                    fx.as_ir_value(bb[gu][ni]), fx.as_ir_value(aa[mi]), fx.as_ir_value(acc[mi][ni][gu]),
                                    4, 4, ni + 2 * (kt % 2), fx.as_ir_value(sb[gu]),
            ...
            exps = []
            for mi in range_constexpr(MR):
                h = [_swiglu_oai(fx.Float32(acc[mi][ni][0][ii]), fx.Float32(acc[mi][ni][1][ii])) for ni in range_constexpr(2) for ii in range_constexpr(4)]
                ...
                col = nbase + (q16 % fx.Int32(2)) * fx.Int32(16) + (q16 // fx.Int32(2)) * fx.Int32(8)
                bop.buffer_store(packed, orsrc, row * fx.Int32(I // 2) + col // fx.Int32(2), offset_is_bytes=True)
            colgrp = nbase // fx.Int32(32)
