...
def compile_tile_map(*, I: int, BM: int = 128):
    NB_N = I // 128
    ...
    def kernel_tile_map(
        ...
        tm_rsrc = _buffer_ops.create_buffer_resource(tile_map, max_size=False, num_records_bytes=(grid_entries + 1) * 4)

        def _eid(i):
            return fx.Int32(_buffer_ops.buffer_load(eid_rsrc, i, vec_width=1, dtype=fx.Int32))
        ...
        for it in range_constexpr(BLOCK_ITERS):
            ...
            if m < vb:
                ...
                for n in range_constexpr(NB_N):
                    _buffer_ops.buffer_store((m << 3) | fx.Int32(n), tm_rsrc, base + cnt * fx.Int32(n))
    ...
    def launch_tile_map(
        ...
        kernel_tile_map(sorted_expert_ids, num_valid_ids, tile_map, num_m_blocks, grid_entries).launch(
...
def tile_map_grid(num_m_blocks: int, I: int) -> int:
    """gemm1 grid for the table: every (m, n) tile of the allocation, rounded to 8 XCDs"""
