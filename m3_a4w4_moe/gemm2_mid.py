...
"""Middle-batch down projection: fp4 VGPR ring, deterministic bf16 partials.

BM32/64 and TN256, four N waves. Output is [token, topk slot, hidden]
with routing weights applied, consumed by the existing moe_reduction kernel.
Every partial has one writer; no atomic accumulation or LDS K loop.
"""
...
def compile_moe_gemm2_mid(*, H, I, E, topk=5, BLOCK_M=32, TILE_N=256, prefetch=3):
    ...
    @flyc.kernel(name=f"gemm2_a4w4_mid_h{H}_i{I}_e{E}_bm{BM}_tn{BN}_pf{prefetch}", known_block_size=[256, 1, 1])
    def kernel(
        ...
        if mb * fx.Int32(BM) < valid_rows:
            ...
            for mi in range_constexpr(MR):
                ...
                for ni in range_constexpr(NI):
                    v = fx.Vector.from_elements([fx.Float32(acc[mi][ni][ii]) * rw[mi] for ii in range_constexpr(4)], fx.Float32).to(fx.BFloat16)
                    col = nbase + fx.Int32(ni * 16) + q16 * fx.Int32(4)
                    bop.buffer_store(v, orsrc, outrow * fx.Int32(H) + col, mask=token < ntok)
