...
"""MiniMax-M3 prefill MoE stage 1 (a4w4), 3-stage variant of ``gemm1.py``.

Same 2x2-wave kernel, but the LDS ring holds THREE K-steps (12 buffers,
144 KB + 16 KB scales = the whole 160 KB LDS of a gfx950 CU, BLOCK_M 128
only) and every DMA batch is issued three K-steps ahead of its use instead
of two (scale gathers four ahead). Why: at 4096-8192 tokens an expert's
W13 slab is touched by only 1-2 m-tiles, so most B loads miss L2 and the
two-step lookahead (~2 us at BLOCK_M 128) is shorter than the loaded HBM
latency; the probe "double the MFMAs per step" shrank the W-miss penalty at
4096 from 42 us to 9 us, i.e. the loss is lookahead, not bandwidth.

Grouped version of ``kernels/gemm/fp4_gemm_4wave.py``.
...
class ScaleGatherMoE:
    ...
    def set_wave_base(self, a_base_row, b_base_row):
        ...
        self._HS = _uniform_i32(
            fx.arith.select(is_a, fx.Int32(self._a_half_groups), fx.Int32(self._b_half_groups))
        )
        self._rsrc = fx.arith.select(is_a, self.a_rsrc, self.b_rsrc)
        self._soff0 = _uniform_i32(fx.Int32(0))

    def gather(self, kstep, slot):
        grp = fx.Int32(self._G) + (self._blk // 2) * fx.Int32(self._HS) + (self._blk % 2)
...
def _intrin_f32(name, args):
    return _f32(_llvm.call_intrinsic(_T.f32, name, [fx.as_ir_value(a) for a in args], [], []))
...
def compile_moe_gemm1_s3(
    *,
    ...
    assert BLOCK_M == 128, "3-stage ring: 144 KB of tiles fits only at BLOCK_M 128"
    assert K % BLOCK_K == 0 and I % LDS_BLOCK_N == 0 and (2 * I) % 256 == 0
    K_ITERS = K // BLOCK_K
    # steps 0, 1, 2 peeled; loop 3 .. K_ITERS-4 issues DMA for kc+3; 3 tail steps.
    UNROLL = 3  # buffer rotation period
    assert K_ITERS >= 6 and (K_ITERS - 6) % UNROLL == 0, K_ITERS
    ...
    N_STAGES = 3
    A_BUFS = 2 * N_STAGES * a_lds_size  # 48 KB
    LDS_TILES_BYTES = A_BUFS + 2 * N_STAGES * b_lds_size  # 144 KB
    assert LDS_TILES_BYTES + _SCALE_LDS_BYTES <= 160 * 1024
    ...
    def kernel_gemm1(
        ...
        # stage s holds K-step j with j % 3 == s: (a half0, a half1, b gate, b up)
        stages = [
            (
                _Buf(_base_ptr, (2 * s + 0) * a_lds_size),
                _Buf(_base_ptr, (2 * s + 1) * a_lds_size),
                _Buf(_base_ptr, A_BUFS + (2 * s + 0) * b_lds_size),
                _Buf(_base_ptr, A_BUFS + (2 * s + 1) * b_lds_size),
            )
        ...
        if block_valid:
                ...
                for rnd in range_constexpr(N_TILES_A):
                    ...
                    sid = fx.Int32(
                        _buffer_ops.buffer_load(
                            ids_rsrc, m_base + half * LDS_BLOCK_M + row, vec_width=1, dtype=fx.Int32
                        )
                    )
                    tok = sid & fx.Int32(0x00FFFFFF)  # padded rows: tok == n_tokens -> OOB -> zeros
            ...
            for g in range_constexpr(4):  # scale gathers 4 K-steps ahead
                scale_gather.gather(g, _slot(g))

            def _issue_batch(stage, kstep):
                sa0, sa1, sb0, sb1 = stage
                a0_g2s.load(sa0, fx.Int32(kstep * A_K_STEP))
                b_g2s.load(sb0, B0_gl_offset + kstep * B_K_STEP)
                b_g2s.load(sb1, B1_gl_offset + kstep * B_K_STEP)
                a1_g2s.load(sa1, fx.Int32(kstep * A_K_STEP))

            _issue_batch(stages[0], 0)
            _issue_batch(stages[1], 1)
            _issue_batch(stages[2], 2)
            NB4 = 2 * N_TILES_A + 2 * N_TILES_B  # one prologue batch (no gather)

            # a_cur0 landed: b0/b1/a1 of batch 0 + batches 1, 2 may fly
            wait_barrier((N_TILES_A + 2 * N_TILES_B) + 2 * NB4)
            a0_frag = a_s2r.load(stages[0][0])
            # b_cur0 AND b_cur1 landed: a1 of batch 0 + batches 1, 2 may fly
            wait_barrier(N_TILES_A + 2 * NB4)
            b0_frag = b_s2r.load(stages[0][2], preshuffled=True)
            b1_frag = b_s2r.load(stages[0][3], preshuffled=True)
            ...
            # Per step kc, in issue order: a0 (NA), b0 (NB), [SEG2], b1 (NB),
            # scale gather (1), a1 (NA) -> P loads, all for K-step kc+3 (the
            # gather for kc+4). Loop-top wait of step kc: K-step kc's batch
            # (issued at step kc-3) complete, steps kc-2 and kc-1 may fly (2P).
            # SEG2 wait of step kc: a0/b0/b1 of K-step kc+1 (issued at step
            # kc-2) complete -> that step's gather + a1 (NA+1), all of step
            # kc-1 (P) and this step's a0 + b0 (NA+NB) may fly. Loads retire in
            # order, so a count one too loose reads LDS before the DMA landed
            # (run-to-run differences), one too tight stalls on a fresh fetch.
            _P = 2 * N_TILES_A + 2 * N_TILES_B + 1
            _MAIN_VMCNT = 2 * _P
            _SEG2_VMCNT = (N_TILES_A + 1) + _P + (N_TILES_A + N_TILES_B)
            # Steps 0 and 1 sit behind the prologue's gather-free batches.
            _STEP0_VMCNT = 2 * NB4
            _STEP0_SEG2_VMCNT = N_TILES_A + NB4 + (N_TILES_A + N_TILES_B)
            _STEP1_VMCNT = NB4 + _P
            _STEP1_SEG2_VMCNT = N_TILES_A + _P + (N_TILES_A + N_TILES_B)
            ...
            def _one_step(kc, a0f, b0f, b1f_in, sc, accs, bufs, zero_acc=False, top_vmcnt=None, seg2_vmcnt=None):
                """bufs = (cur, next, next2): K-steps kc, kc+1, kc+2. DMA of
                K-step kc+3 goes into cur (its a0/b0/b1 were moved to registers
                at step kc-1, its a1 is read below before the SEG2 barrier)."""
                ...
                _b0n = [None] * N_TILES_B
                _b1n = [None] * N_TILES_B
                ak = (kc_i + fx.Int32(3)) * fx.Int32(A_K_STEP)
                bk = (kc_i + fx.Int32(3)) * fx.Int32(B_K_STEP)
                a0_off = ak
                a1_off = ak
                ...
                _gk = _min(kc_i + fx.Int32(4), fx.Int32(K_ITERS - 1))
            ...
            a0f, b0f, b1f, sc, accs, bufs = _one_step(
                1, a0f, b0f, b1f, sc, accs, bufs, top_vmcnt=_STEP1_VMCNT, seg2_vmcnt=_STEP1_SEG2_VMCNT
            )
            a0f, b0f, b1f, sc, accs, bufs = _one_step(2, a0f, b0f, b1f, sc, accs, bufs)
            # three rotations -> bufs == bufs0 again at the loop top
            ...
            for kk, state in range(3, K_ITERS - 3, UNROLL, init=init_state):
                off = 0
            ...
            # ---- tail: K-steps K-3, K-2, K-1 live in stages 0, 1, 2; no DMA ----
            # step K-3: batch K-3 (issued at K-6) complete -> steps K-5, K-4 fly
            saR0, saR1, sbC0, sbC1 = sc
            _scn = [None, None, None, None]
            _rd_scn = _read_scale_thunks(fx.Int32(K_ITERS - 2), _scn)
            _a1 = [None] * N_TILES_A
            wait_barrier(2 * _P)
            il = _s2r_thunks(a_s2r, stages[0][1], _a1, N_TILES_A, False) + _rd_scn[:2]
            c00_frag = mfma.call(a0_frag, b0_frag, c00_frag, saR0, sbC0, interleave=il)
            c01_frag = mfma.call(a0_frag, b1_frag, c01_frag, saR0, sbC1, interleave=_rd_scn[2:])
            a1_frag = _a1
            _a0n = [None] * N_TILES_A
            _b0n = [None] * N_TILES_B
            _b1n = [None] * N_TILES_B
            # a0/b0/b1 of K-step K-2 (issued at K-5) -> its gather + a1, step K-4 fly
            wait_barrier((N_TILES_A + 1) + _P)
            il = (
                _s2r_thunks(a_s2r, stages[1][0], _a0n, N_TILES_A, False)
                + _s2r_thunks(b_s2r, stages[1][2], _b0n, N_TILES_B, True)
                + _s2r_thunks(b_s2r, stages[1][3], _b1n, N_TILES_B, True)
            )
            ...
            # step K-2: batch K-2 complete -> step K-4 (for K-1) flies
            saR0, saR1, sbC0, sbC1 = (_scn[0], _scn[1], _scn[2], _scn[3])
            _scn2 = [None, None, None, None]
            _rd_scn2 = _read_scale_thunks(fx.Int32(K_ITERS - 1), _scn2)
            _a1b = [None] * N_TILES_A
            wait_barrier(_P)
            il = _s2r_thunks(a_s2r, stages[1][1], _a1b, N_TILES_A, False) + _rd_scn2[:2]
            c00_frag = mfma.call(a0_frag, b0_frag, c00_frag, saR0, sbC0, interleave=il)
            c01_frag = mfma.call(a0_frag, b1_frag, c01_frag, saR0, sbC1, interleave=_rd_scn2[2:])
            a1_frag = _a1b
            _a0n2 = [None] * N_TILES_A
            _b0n2 = [None] * N_TILES_B
            _b1n2 = [None] * N_TILES_B
            # a0/b0/b1 of K-step K-1 -> only its gather + a1 fly
            wait_barrier(N_TILES_A + 1)
            il = (
                _s2r_thunks(a_s2r, stages[2][0], _a0n2, N_TILES_A, False)
                + _s2r_thunks(b_s2r, stages[2][2], _b0n2, N_TILES_B, True)
                + _s2r_thunks(b_s2r, stages[2][3], _b1n2, N_TILES_B, True)
            )
            c10_frag = mfma.call(a1_frag, b0_frag, c10_frag, saR1, sbC0, interleave=il)
            c11_frag = mfma.call(a1_frag, b1_frag, c11_frag, saR1, sbC1)
            a0_frag = _a0n2
            ...
            # step K-1: everything landed
            _a1c = [None] * N_TILES_A
            wait_barrier(0)
            saR0, saR1, sbC0, sbC1 = (_scn2[0], _scn2[1], _scn2[2], _scn2[3])
            il = _s2r_thunks(a_s2r, stages[2][1], _a1c, N_TILES_A, False)
            c00_frag = mfma.call(a0_frag, b0_frag, c00_frag, saR0, sbC0, interleave=il)
            a1_frag = _a1c
            ...
            def _epilogue(c_gate, c_up, base_row):
                """One quadrant pair (rows base_row + ti*16 + r16, wave's 64 gate cols)."""
