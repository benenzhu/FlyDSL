...
from m3_a4w4_moe.lowlevel import (
    extract_global_ptr,
    global_load_f32,
    global_load_i32,
    global_load_i32_vec,
...
def compile_moe_sort(*, E: int, topk: int, block_m: int, sort_ctas: int = 32, threads: int = 1024, stages: int = 3):
    ...
    allocator = SmemAllocator(None, arch=get_rocm_arch(), global_sym_name=f"smem_m3_sort_E{E}")
    ...
    def _pairs_and_slice(n_tokens_i32, bx):
        ...
        per_cta = (total + arith.constant(sort_ctas - 1, index=True)) >> arith.constant(ctas_shift, index=True)
        c_start = bx * per_cta
    ...
    def sort_count(arg_topk_ids: fx.Pointer, arg_block_offsets: fx.Pointer, arg_n_tokens: fx.Int32):
        i32 = T.i32
        tx = gpu.thread_id("x")
        bx = gpu.block_id("x")
    ...
    def sort_cumsum(
        ...
        expert_starts = SmemPtr(base_ptr, lds_starts_offset, T.i32, shape=(E + 1,)).get()
        ...
        with ir.InsertionPoint(_if_sum.then_block):
            ...
            for c_group in range_constexpr(sort_ctas // 4):
                cnts = global_load_i32_vec(offsets_ptr, e_base + arith.constant(c_group * 4, type=i32), 4)
                for c_lane in range_constexpr(4):
                    total = total + ArithValue(vector.extract(cnts, dynamic_position=[], static_position=[c_lane]))
        ...
        with ir.InsertionPoint(_if_t0.then_block):
            scan = scf.ForOp(arith.index(0), arith.index(E), arith.index(1), [arith._to_raw(c0_i32)])
    ...
    def sort_place_pad(
        ...
        _if_init = scf.IfOp(arith.cmpi(CmpIPredicate.ult, tx, arith.constant(E, index=True)))
        ...
        _if_pad = scf.IfOp(arith.cmpi(CmpIPredicate.ult, tx, arith.constant(block_m, index=True)))
        with ir.InsertionPoint(_if_pad.then_block):
            for ee in range_constexpr(experts_per_cta):
                ...
                _if_e = scf.IfOp(arith.cmpi(CmpIPredicate.ult, e, arith.constant(E, index=True)))
                with ir.InsertionPoint(_if_e.then_block):
                    ...
                    padded_end = ArithValue(memref.load(row_starts, [e + arith.index(1)]))
    ...
    def launch_sort(
        ...
        if const_expr(stages >= 2):
            sort_cumsum(block_offsets, masked_m, real_counts, cumsum, sorted_expert_ids).launch(
                grid=(1, 1, 1), block=(threads, 1, 1), stream=stream
            )
        if const_expr(stages >= 3):
            sort_place_pad(
                topk_ids, topk_weight, block_offsets, real_counts, cumsum, sorted_ids, reverse_sorted, sorted_weights,
                m_indices, n_tokens,
...
def moe_sort_3stage(topk_ids, topk_weights, E, block_m, *, sort_ctas=32, threads=1024, bufs=None):
    ...
    if bufs is None:
        bufs = SortBuffers.allocate(n_tokens, E, topk, block_m, sort_ctas, topk_ids.device)
    launch = compile_moe_sort(E=E, topk=topk, block_m=block_m, sort_ctas=sort_ctas, threads=threads)
    launch(*bufs.launch_args(topk_ids.contiguous(), topk_weights.contiguous().float(), n_tokens))
    return bufs
