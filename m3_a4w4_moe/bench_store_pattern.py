...
def _scatter(out_ptr, rows_ptr, cols_ptr, n_pieces, SEG: tl.constexpr, PER_PROG: tl.constexpr, ROW_BYTES: tl.constexpr):
    ...
    for i in range(PER_PROG):
        ...
        tl.store(ptrs.to(tl.pointer_type(tl.int32)), v)
...
def main():
    ...
    for seg in (128, 256, 512, 1024, 2048, 4096):
        ...
        grid = (n // per_prog,)
        for _ in range(2):
            _scatter[grid](out, rows, cols, n, SEG=seg, PER_PROG=per_prog, ROW_BYTES=a.hbytes, num_warps=1)
        torch.cuda.synchronize()
        ts = []
        for _ in range(a.reps):
            s = torch.cuda.Event(enable_timing=True); e = torch.cuda.Event(enable_timing=True)
            s.record(); _scatter[grid](out, rows, cols, n, SEG=seg, PER_PROG=per_prog, ROW_BYTES=a.hbytes, num_warps=1); e.record()
            torch.cuda.synchronize(); ts.append(s.elapsed_time(e) * 1e3)
        t = statistics.median(ts)
        print(f"seg {seg:5d} B: {n/1e6:6.1f} M pieces, {t:8.1f} us, {total/t/1e6:5.2f} TB/s, {n/t/1e3:6.1f} G pieces/s", flush=True)
