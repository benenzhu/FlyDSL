...
def _elem_ptr(global_ptr, elem_offset, elem_bytes):
    byte_offset_i64 = elem_offset_to_i64(elem_offset) * arith.constant(elem_bytes, type=T.i64)
    return buffer_ops.get_element_ptr(global_ptr, byte_offset=byte_offset_i64, elem_type=T.i8)
...
def global_load_i32(global_ptr, elem_offset, *, nontemporal=False):
    return ArithValue(llvm.LoadOp(T.i32, _elem_ptr(global_ptr, elem_offset, 4), alignment=4, nontemporal=nontemporal).result)


def global_load_f32(global_ptr, elem_offset, *, nontemporal=False):
    return ArithValue(llvm.LoadOp(T.f32, _elem_ptr(global_ptr, elem_offset, 4), alignment=4, nontemporal=nontemporal).result)


def global_load_i32_vec(global_ptr, elem_offset, width, *, nontemporal=False):
    return llvm.LoadOp(T.vec(width, T.i32), _elem_ptr(global_ptr, elem_offset, 4), alignment=4, nontemporal=nontemporal).result


def global_store_i32(global_ptr, elem_offset, value, *, nontemporal=False):
    return llvm.StoreOp(_raw(value), _elem_ptr(global_ptr, elem_offset, 4), alignment=4, nontemporal=nontemporal)
...
def lds_i32_ptr(lds_memref, elem_offset):
    byte_offset_idx = ArithValue(elem_offset_to_i64(elem_offset) * arith.constant(4, type=T.i64)).index_cast(T.index)
