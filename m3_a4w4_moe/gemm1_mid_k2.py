...
"""Four-wave mid-batch gate/up: two N waves by two K waves, AGPR C.

Input/output layouts match gemm1.py. Unlike its 2x2-wave LDS pipeline, every
wave owns 64 gate columns and their up columns; A/W flow directly to VGPRs.
...
Experimental checkpoint: the initial GPU compilation failed because the IR
module name was shadowed by the sorted-ID buffer. That name is fixed below;
GPU compilation, numerical validation and timing still need to be run.
"""
...
def compile_moe_gemm1_mid(*, H, I, E, BLOCK_M=32, prefetch=2):
    assert BLOCK_M in (32, 64) and H % 256 == 0 and I % 128 == 0
    BM, BN, KT = BLOCK_M, 128, H // 2 // 128
    assert H % 512 == 0
    assert I % BN == 0
    NI = BN // 2 // 16
    MR = BM // 16
    ...
    @fx.struct
    class Shared:
        raw: fx.Array[fx.Uint8, 4 * NACC * 64 * 16, 16]

    @flyc.kernel(name=f"gemm1_a4w4_mid_k2_h{H}_i{I}_e{E}_bm{BM}_pf{prefetch}", known_block_size=[256, 1, 1])
    def kernel(
        ...
        wn, wk = wave % fx.Int32(2), wave // fx.Int32(2)
        ...
        if mb * fx.Int32(BM) < valid_rows:
            ...
            nbase = nb * fx.Int32(BN) + wn * fx.Int32(NI * 16)
            ...
            flat = _pin_accumulators([acc[mi][ni][gu] for mi in range(MR) for ni in range(NI) for gu in range(2)])
            scratch = llvm.inttoptr(_ir.Type.parse("!llvm.ptr<3>"), fx.as_ir_value(fx.Int32(fx.ptrtoint(smem))))
            for ai in range_constexpr(NACC):
                off = (wave * fx.Int32(NACC * 256) + fx.Int32(ai * 256) + lane * fx.Int32(4)) * fx.Int32(4)
                ptr = bop.get_element_ptr(scratch, byte_offset=fx.as_ir_value(off), elem_type=T.i8)
                llvm.StoreOp(fx.as_ir_value(flat[ai]), ptr, alignment=16)
            gpu.barrier()
            peer = wave ^ fx.Int32(2)
            for mi in range_constexpr(MR):
                for ni in range_constexpr(NI):
                    for gu in range_constexpr(2):
                        ai = (mi * NI + ni) * 2 + gu
                        off = (peer * fx.Int32(NACC * 256) + fx.Int32(ai * 256) + lane * fx.Int32(4)) * fx.Int32(4)
                        ptr = bop.get_element_ptr(scratch, byte_offset=fx.as_ir_value(off), elem_type=T.i8)
                        other = fx.Vector(llvm.load(T.vec(4, T.f32), ptr, alignment=16))
                        own = fx.Vector(flat[ai])
                        acc[mi][ni][gu] = fx.Vector.from_elements([own[ii] + other[ii] for ii in range_constexpr(4)], fx.Float32)
            for np in range_constexpr(NI // 2):
                ...
                for mi in range_constexpr(MR):
                    ...
                    bop.buffer_store(packed, orsrc, row * fx.Int32(I // 2) + col // fx.Int32(2), offset_is_bytes=True, mask=wk == fx.Int32(0))
                ...
                for mp in range_constexpr(MR // 2):
                    ...
                    bop.buffer_store(pair, osr, off, offset_is_bytes=True, mask=(q16 == fx.Int32(0)) & (wk == fx.Int32(0)))
