...
"""Middle-batch gate/up: deep weight ring, shallow L2-hot activation ring.
...
def compile_moe_gemm1_mid(*, H, I, E, BLOCK_M=32, prefetch=3):
    ...
    @flyc.kernel(name=f"gemm1_a4w4_mid_splitring_h{H}_i{I}_e{E}_bm{BM}_pf{prefetch}", known_block_size=[256, 1, 1])
    def kernel(
            ...
            def load_b_tile(kt, prev):
                ...
                if const_expr(kt % 2 == 0):
                    sb = [bop.buffer_load(wsr, q16 * fx.Int32(16) + l16, vec_width=1, dtype=fx.Int32,
                                         soffset_bytes=((expert * fx.Int32(2 * I // 32) + nbase // fx.Int32(32) + fx.Int32(gu * I // 32)) * fx.Int32(H // 256 * 64) + fx.Int32(kt // 2 * 64)) * fx.Int32(4)) for gu in range_constexpr(2)]
                else:
                    sb = prev[1]
                return bb, sb
