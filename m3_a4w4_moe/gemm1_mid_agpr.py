...
"""Four-wave mid-batch gate/up with explicitly pinned AGPR accumulators.
...
def compile_moe_gemm1_mid(*, H, I, E, BLOCK_M=32, prefetch=3):
    ...
    @flyc.kernel(name=f"gemm1_a4w4_mid_agpr_h{H}_i{I}_e{E}_bm{BM}_pf{prefetch}", known_block_size=[256, 1, 1])
    def kernel(
        ...
        if mb * fx.Int32(BM) < valid_rows:
            ...
            mfma = Mfma16x16x128Fp4(MR, 2)
            acc = [[[None for gu in range(2)] for ni in range(2)] for mi in range(MR)]
            ring = []
                    ...
                    for ni in range_constexpr(2):
                        for gu in range_constexpr(2):
                            acc[mi][ni][gu] = mfma._mfma_agpr(
                                aa[mi], bb[gu][ni], acc[mi][ni][gu], sa[mi // 2], sb[gu],
                                kt % 2, mi % 2, ni,
                            )
